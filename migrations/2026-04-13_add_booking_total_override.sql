...
ALTER TABLE bookings
  ADD COLUMN IF NOT EXISTS custom_total_price NUMERIC(10,2),
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'chk_booking_custom_total'
      AND conrelid = 'bookings'::regclass
  ) THEN
    ALTER TABLE bookings
      ADD CONSTRAINT chk_booking_custom_total
      CHECK (custom_total_price IS NULL OR custom_total_price >= 0);
  END IF;
END$$;
...
AS $$
BEGIN
  RETURN create_booking_with_allocations(
    p_customer_id,
    p_start,
    p_end,
    p_category_ids,
    ...
    NULL,
    NULL,
    p_custom_total_prices,
    p_custom_price_notes
  );
END$$;
