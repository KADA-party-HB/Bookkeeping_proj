ALTER TABLE furnishing_categories
ADD COLUMN IF NOT EXISTS setup_service_fee NUMERIC(10,2);

UPDATE furnishing_categories
SET setup_service_fee = 0.00
WHERE setup_service_fee IS NULL;

ALTER TABLE furnishing_categories
ALTER COLUMN setup_service_fee SET DEFAULT 0.00;

ALTER TABLE furnishing_categories
ALTER COLUMN setup_service_fee SET NOT NULL;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'chk_furncat_setup_fee'
  ) THEN
    ALTER TABLE furnishing_categories
    ADD CONSTRAINT chk_furncat_setup_fee CHECK (setup_service_fee >= 0);
  END IF;
END$$;
